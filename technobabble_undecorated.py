...
def get_error_text():
    ...
    text = my_task[1].capitalize() + my_target + '...'
    secondary_text = ''

    rand = random.randint(1,3)
    if rand == 1:
        secondary_text = 'WARNING: ' + my_task[1] + my_target + ' ' + random.choice(errors) + '.'
    if rand == 2:
        secondary_text = 'Failed to ' + str(my_task[0] + my_target + '.')
    if rand == 3:
        secondary_text = "!!! Couldn't " + str(my_task[0]) + my_target + '. !!!'
    return (text, secondary_text)

def get_simple_text():
    # simply create a colored string combining the words in our lists. 
    return random.choice(process)[1].capitalize() + ' ' + random.choice(things) + "..."

def get_flavorful_text():
    # this is the same as get_simple_text, except it adds and extra descriptive word in the middle. 
    return random.choice(process)[1].capitalize() + ' ' + random.choice(descriptive) + ' ' + random.choice(things) + "..."
...
def get_speed_text():
    ...
    if chance == 1:
        text = my_task[1].capitalize() + ' ' + my_target + '...'
        secondary_text = my_task[1].capitalize() + ' at ' + str(timer) + unit + '/s.'
        return(secondary_text,text)
    elif chance == 2:
        text = my_task[1].capitalize() + ' ' + my_target + '...'
        secondary_text = my_target.capitalize()  + ' used ' + str(timer) + unit + ' of additional disk space.'
        return(text,secondary_text)    
    else:
        text = my_task[1].capitalize() + ' ' + my_target + '...'
        secondary_text = my_target.capitalize()  + ' used ' + str(timer) + unit + ' of memory.'
        return(text,secondary_text)
...
def get_timed_text():
    ...
    my_target = random.choice(things)
    text = my_task[1].capitalize() + ' ' + my_target + '...'
    secondary_text = my_target.capitalize() + ' ' + my_task[2] + ' in ' + str(timer) + 'ms.'
...
def get_directory_text():
    chance = random.randint(1,3)
    if chance == 1:
        return 'saved in ' + get_directory() + '.'
    elif chance == 2:
        return 'Loading from ' + get_directory() + '...'
    else:
        return 'Scanning ' + get_directory() + '.'
